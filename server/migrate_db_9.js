...
async function migrate() {
  ...
  try {
    console.log('Iniciando migración v9: Añadir real_match_datetime a la tabla de partidos...');
    
    await addColumnIfNotExists(client, 'matches', 'real_match_datetime', 'TIMESTAMP WITH TIME ZONE NULL');

    console.log('Migración v9 completada con éxito.');
  } catch (err) {
    console.error('Error durante la migración v9:', err);
  } finally {
    client.release();
  }
}

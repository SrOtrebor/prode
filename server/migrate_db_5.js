...
async function migrate() {
  ...
  try {
    console.log('Iniciando migración v5: Añadir vip_expires_at a la tabla de usuarios...');
    
    await addColumnIfNotExists(client, 'users', 'vip_expires_at', 'TIMESTAMP WITH TIME ZONE NULL');

    console.log('Migración v5 completada con éxito.');
  } catch (err) {
    console.error('Error durante la migración v5:', err);
  } finally {
    client.release();
  }
}
